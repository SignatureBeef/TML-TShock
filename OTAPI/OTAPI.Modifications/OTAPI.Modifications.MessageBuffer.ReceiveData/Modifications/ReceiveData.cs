...
namespace OTAPI.Patcher.Engine.Modifications.Hooks.Net
{
	public class ReceiveData : ModificationBase
	{
		...
		public override void Run()
		{
			...
			var stored = vanilla.Body.Instructions.First(i => (i.Operand as MethodReference)?.Name == "ReadString").Next;

			var il = vanilla.Body.GetILProcessor();

			//il.InsertAfter(stored, il.Create(OpCodes.Call, vanilla.Module.Import(
			//	typeof(System.Console).GetMethod("WriteLine", new[] {
			//	typeof(string)
			//	})
			//	)));
			//il.InsertAfter(stored, il.Create(OpCodes.Ldloc_S, (VariableDefinition)stored.Operand));
			...
			var target = vanilla.Body.Instructions.First(x => x.OpCode == OpCodes.Stloc_1
				//After read buffer
			...
			var call = il.Create(OpCodes.Call, vanilla.Module.Import(callback));
			il.InsertAfter(target, call);

			//il.InsertBefore(call, il.Create(OpCodes.Ldind_I4));
			il.InsertAfter(call, il.Create(OpCodes.Pop));
			//il.Remove(target.Next);
			...
			il.InsertBefore(call, il.Create(OpCodes.Ldloca, vanilla.Body.Variables.First(x => x.VariableType.Name == "Byte"))); //First byte is b
			il.InsertBefore(call, il.Create(OpCodes.Ldloca, vanilla.Body.Variables.First(x => x.VariableType.Name == "Int32"))); //First int is read offset

			//Add parameters by reference
			foreach (var prm in vanilla.Parameters)
			{
				if (prm.Name != "messageType")
					il.InsertBefore(call, il.Create(OpCodes.Ldarga, prm));
			}
		}
	}
}
